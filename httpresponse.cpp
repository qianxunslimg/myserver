...
const unordered_map<string, string> HttpResponse::SOURCE_FOLDER = {
    {".html", "/html"}, {".xml", "/file"},   {".xhtml", "/html"},
    {".txt", "/file"},  {".rtf", "/file"},   {".pdf", "/file"},
    {".word", "/file"}, {".png", "/image"},  {".gif", "/image"},
    {".ico", "/html"},  {".jpg", "/image"},  {".jpeg", "/image"},
    {".au", "/music"},  {".mpeg", "/video"}, {".mpg", "/video"},
    {".avi", "/video"}, {".gz", "/file"},    {".tar", "/file"},
    {".css", "/file"},  {".js", "/file"},    {".flac", "/music"},
    {".lrc", "/music"}, {".mp4", "/video"},
};
...
void HttpResponse::encode_str(std::string &from) {
  ...
  for (size_t i = 0; i < length; i++) {
    if (isalnum((unsigned char)from[i]) || (from[i] == '/') ||
        (from[i] == '-') || (from[i] == '_') || (from[i] == '.') ||
        (from[i] == '~'))
      strTemp += from[i];
    else if (from[i] == ' ')
      strTemp += "+";

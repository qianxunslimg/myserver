...
WebServer::WebServer(config *cfgObj)
    : port_(cfgObj->port_), openLinger_(cfgObj->optLinger_),
      timeoutS_(cfgObj->timeoutS_), isClose_(false) {
  ...
  if (cfgObj->openLog_) {
    ...
    if (isClose_) {
      ...
      LOG_INFO("Port:%d, OpenLinger: %s", cfgObj->port_,
               cfgObj->optLinger_ ? "true" : "false");
      LOG_INFO("Listen Mode: %s, OpenConn Mode: %s",
               (listenEvent_ & EPOLLET ? "ET" : "LT"),
               (connEvent_ & EPOLLET ? "ET" : "LT"));
      LOG_INFO("LogSys Open: %d", cfgObj->openLog_);
      LOG_INFO("srcDir: %s", HttpConn::srcDir);
      LOG_INFO("SqlConnPool num: %d, ThreadPool num: %d", cfgObj->sqlPoolNum_,
               cfgObj->threadPoolNum_);
    }
  }
...
void WebServer::ExtentTime_(HttpConn *client) {
  assert(client);
  if (timeoutS_ > 0) {
    timer_->updateNode(client->GetFd(), timeoutS_);
  }
}

...
const unordered_set<string> HttpRequest::DEFAULT_HTML_REQUEST{
    //页面信息
    "/index.html", "/register.html",  "/login.html",     "/welcome.html",
    "/test.html",  "/testVideo.html", "/testMusic.html", "/testImage.html",
};

const unordered_map<string, int> HttpRequest::DEFAULT_HTML_POST_REQUEST{
    {"/registerSubmit", 0}, {"/loginSubmit", 1}, {"/uploadfile", 2}};
      ...
      if (tag == 0 || tag == 1) {
        ...
        default:
          ...
          break;
             ...
             header_["Content-Type"].find("multipart/form-data") !=
                 string::npos) //对上传的文件进行解析
  {
    if (DEFAULT_HTML_POST_REQUEST.count(path_) && ParseBodyForFile())
      path_ = "upload_ok";
    else
      path_ = "upload_err";
  }
...
bool HttpRequest::ParseBodyForFile() {
  ...
  while (true) {
    ...
    if (line.find("filename") != string::npos) {
      ...
      const char *temp =
          search(line.data(), line.data() + line.size(), targ, targ + 8);
  ...
  const char *temp = search(
      body_->Peek(), body_->Peek() + body_->ReadableBytes(), targ, targ + 6);
  if (temp)
    body_->RecycleFrom(temp);
...
bool HttpRequest::ParseBodyForUserInfo() {
  ...
  char password[100];
  if (-1 !=
      sscanf(body_->Peek(), "%*[^=]=%[^&]&%*[^=]=%[^=]", name, password)) {
...
bool HttpRequest::SaveFile(char filename[]) {
  if (fileSaveDir_.empty())
    LOG_DEBUG("httprequest: fileSaveDir_ = s%", fileSaveDir_);

  ofstream FILE_save;
  FILE_save.open(fileSaveDir_ + "/" + filename);
  if (FILE_save.is_open()) {
    FILE_save << body_->Peek();
    LOG_INFO("httprequest: save file successed ! dir : %s",
             fileSaveDir_ + "/" + filename);
  } else {
...
sqlVerifyState HttpRequest::UserVerify(const std::string &name,
  ...
  snprintf(order, 256,
           "SELECT username, passwd FROM user WHERE username='%s' LIMIT 1",
           name.c_str());
  LOG_DEBUG("%s", order);
